...
mySITES="https://hub.docker.com https://github.com https://pypi.python.org"
myTPOTCOMPOSE="/opt/tpot/etc/tpot.yml"
myLSB_STABLE_SUPPORTED="stretch buster"
myLSB_TESTING_SUPPORTED="stable"
myREMOTESITES="https://hub.docker.com https://github.com https://pypi.python.org"
myPREINSTALLPACKAGES="aria2 httpd-tools cracklib-runtime curl dialog figlet fuse grc libcrack2 libpq-dev net-tools "
myINSTALLPACKAGES="aria2 httpd-tools make automake gcc gcc-c++ kernel-devel bash-completion ca-certificates cockpit-ws console-setup console-setup-linux cracklib-runtime curl dialog dnsutils docker.io docker-compose elasticsearch-curator ethtool fail2ban figlet genisoimage git glances grc haveged html2text htop iptables iw jq kbd libcrack2 libltdl7 libpam-google-authenticator man mosh multitail netselect-apt net-tools npm ntp openssh-server openssl pass pigz prips software-properties-common syslinux psmisc pv python3-pip unattended-upgrades unzip vim wget wireless-tools wpasupplicant"
myINFO="\
###########################################
...
27 16 * * 0     root    yum update -y && sleep 10 && reboot
"
...
function fuCHECKPACKAGES {
  ...
  if [ "$myCURL" == "" ] || [ "$myWGET" == "" ] || [ "$mySUDO" == "" ]
    then
      echo "### Installing deps for yum  apt-fast"
      yum -y update
      yum -y install curl wget sudo
  fi
  echo "### Installing yum apt-fast"
...
#  apt-fast -y dist-upgrade -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" --force-yes
  ...
  echo
  yum -y install $myINSTALLPACKAGES
  # Remove exim4
  echo "### Removing and holding back problematic packages ..."
  yum -y remove exim4-base mailutils pcp cockpit-pcp postfix
...
groupadd tpot -g 2000 
useradd -M -u 2000 -s /sbin/nologin -g tpot tpot
...
fuBANNER "Update IP"
sed -i 's/toilet -f ivrit -F metal --filter border:metal/figlet/g' /opt/tpot/bin/updateip.sh
...
if [ "$myTPOT_DEPLOYMENT_TYPE" == "auto" ];
  then
    ...
    fuBANNER "Rebooting ..."
    sleep 2
    sed -i 's/iptables-legacy/iptables/g' /opt/tpot/bin/rules.sh
    sed -i 's/TimeoutSec=infinity/TimeoutSec=600/g' /etc/systemd/system/tpot.service
    systemctl daemon-reload
    reboot
fi
